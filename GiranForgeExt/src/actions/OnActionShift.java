...
public class OnActionShift extends Functions {
		...
		if (player != null && gameObject != null) {
			...
			if (gameObject.isNpc()) {
				...
				if (!Config.ALT_FULL_NPC_STATS_PAGE) {
					string = HtmCache.getInstance().getNotNull("scripts/actions/player.L2NpcInstance.onActionShift.htm",
							player);
				} else {
					Spawner spawner;
					string = HtmCache.getInstance()
							.getNotNull("scripts/actions/player.L2NpcInstance.onActionShift.full.htm", player);
					if (npcInstance.getSpawn() != null && (spawner = npcInstance.getSpawn()) instanceof HardSpawner) {
						HardSpawner hardSpawner = (HardSpawner) spawner;
						SpawnTemplate spawnTemplate = hardSpawner.getTemplate();
						string = string.replaceFirst("%maker%",
								spawnTemplate != null && spawnTemplate.getMakerName() != null
										? spawnTemplate.getMakerName()
										: "");
					...
					string = string.replaceFirst("%class%",
							npcInstance.getClass().getSimpleName().replaceFirst("L2", "").replaceFirst("Instance", ""));
					string = string.replaceFirst("%id%", String.valueOf(npcInstance.getNpcId()));
					string = string.replaceFirst("%respawn%",
							String.valueOf(npcInstance.getSpawn() != null
									? Util.formatTime((int) ((int) (npcInstance.getSpawn().getRespawnCron() != null
											? (npcInstance.getSpawn().getRespawnCron().next(System.currentTimeMillis())
													- System.currentTimeMillis()) / 1000L
											: this.a(npcInstance.getSpawn().getRespawnDelay(), npcInstance))))
									: "0"));
					...
					string = string.replaceFirst("%dist%",
							String.valueOf((int) npcInstance.getDistance3D((GameObject) player)));
					...
					int var10002 = npcInstance.getLoc().x;
					string = string.replaceFirst("%xyz%",
							var10002 + " " + npcInstance.getLoc().y + " " + npcInstance.getLoc().z);
					string = string.replaceFirst("%ai_type%", npcInstance.getAI().getClass().getSimpleName());
					string = string.replaceFirst("%direction%",
							PositionUtils.getDirectionTo((Creature) npcInstance, (Creature) ((Object) player))
				...
				string = string
						.replaceFirst("%respawn%",
								String.valueOf(npcInstance.getSpawn() != null
										? Util.formatTime((int) ((int) (npcInstance.getSpawn().getRespawnCron() != null
												? (npcInstance.getSpawn().getRespawnCron()
														.next(System.currentTimeMillis()) - System.currentTimeMillis())
														/ 1000L
												: this.a(npcInstance.getSpawn().getRespawnDelay(), npcInstance))))
										: "0"));
				...
				if (player.isGM()) {
					String var14 = String.valueOf(npcInstance.getAI());
					var10000 = string.replaceFirst("%AI%", var14 + ",<br1>active: " + npcInstance.getAI().isActive()
							+ ",<br1>intention: " + npcInstance.getAI().getIntention());
				} else {
					var10000 = string.replaceFirst("%AI%", "");
				}
				string = var10000;
				OnActionShift.show((String) string, (Player) player, (NpcInstance) npcInstance,
						(Object[]) new Object[0]);
			}
			return true;
		}
	...
	public String getNpcRaceById(int n) {
		...
		case 18 -> "Dwarves";
		case 19 -> "Others";
		case 20 -> "Non-living Beings";
		case 21 -> "Siege Weapons";
		case 22 -> "Defending Army";
		case 23 -> "Mercenaries";
		case 24 -> "Unknown Creature";
		case 25 -> "Kamael";
		default -> "Not defined";
		};
	}
	...
	public void droplist(Player player, NpcInstance npcInstance) {
		if (player != null && npcInstance != null) {
			if (GiranForgeConfig.OVERRIDE_SHIFT_CLICK && GiranForgeConfig.ENABLE_DATABASE) {
				DatabaseMonster.getInstance().getMobData(player, npcInstance.getNpcId());
				return;
			}
			if (Config.ALT_GAME_SHOW_DROPLIST) {
				RewardListInfo.showRewardHtml((Player) player, (NpcInstance) npcInstance);
			}
		}
	}
	...
	public void quests() {
		...
		if (player != null && npcInstance != null) {
			...
			Map<QuestEventType, Quest[]> map = npcInstance.getTemplate().getQuestEvents();
			for (Map.Entry entry : map.entrySet()) {
				Quest[] var7 = (Quest[]) entry.getValue();
				int var8 = var7.length;
				for (Quest quest : var7) {
					stringBuilder.append(entry.getKey()).append(" ").append(quest.getClass().getSimpleName())
							.append("<br1>");
				}
			}
			stringBuilder.append("</body></html>");
			OnActionShift.show((String) stringBuilder.toString(), (Player) player, (NpcInstance) npcInstance,
					(Object[]) new Object[0]);
		}
	}
	...
	public void skills() {
		...
		if (player != null && npcInstance != null) {
			...
			Collection<Skill> collection = npcInstance.getAllSkills();
	...
	public void effects() {
		...
		if (player != null && npcInstance != null) {
			StringBuilder stringBuilder = new StringBuilder("<html><body><center><font color=\"LEVEL\">");
			stringBuilder.append(this.a(npcInstance)).append("<br></font></center><br>");
			List<Effect> list = npcInstance.getEffectList().getAllEffects();
	...
	public void resists() {
		...
		if (player != null && npcInstance != null) {
			...
			bl |= this.a(stringBuilder, "Sword",
					100.0 - npcInstance.calcStat(Stats.SWORD_WPN_VULNERABILITY, null, null));
			bl |= this.a(stringBuilder, "Dual Sword",
					100.0 - npcInstance.calcStat(Stats.DUAL_WPN_VULNERABILITY, null, null));
			bl |= this.a(stringBuilder, "Blunt",
					100.0 - npcInstance.calcStat(Stats.BLUNT_WPN_VULNERABILITY, null, null));
			bl |= this.a(stringBuilder, "Dagger",
					100.0 - npcInstance.calcStat(Stats.DAGGER_WPN_VULNERABILITY, null, null));
			bl |= this.a(stringBuilder, "Bow", 100.0 - npcInstance.calcStat(Stats.BOW_WPN_VULNERABILITY, null, null));
			if (!((bl |= this.a(stringBuilder, "Polearm",
					100.0 - npcInstance.calcStat(Stats.POLE_WPN_VULNERABILITY, null, null)))
					| this.a(stringBuilder, "Fist",
							100.0 - npcInstance.calcStat(Stats.FIST_WPN_VULNERABILITY, null, null)))) {
	...
	public void aggro(int n) {
		...
		if (player != null && npcInstance != null) {
			...
			for (int i = n5; i < n6; ++i) {
				...
				stringBuilder.append("<tr><td>").append(hateInfo.attacker.getName()).append("</td><td>")
						.append(hateInfo.damage).append("</td><td>").append(hateInfo.hate).append("</td></tr>");
			}
			stringBuilder.append("</table><br><center>");
			if (n > 1) {
				stringBuilder
						.append("<button value=\"Previous\" action=\"bypass -h scripts_actions.OnActionShift:aggro ")
						.append(n - 1)
						.append("\" width=100 height=15 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" />");
			}
			if (n < n4) {
				stringBuilder.append("<button value=\"Next\" action=\"bypass -h scripts_actions.OnActionShift:aggro ")
						.append(n + 1)
						.append("\" width=100 height=15 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" />");
			}
	...
	public boolean OnActionShift_DoorInstance(Player player, GameObject gameObject) {
		if (player != null && gameObject != null && player.getPlayerAccess().Door && gameObject.isDoor()) {
			...
			String string = HtmCache.getInstance().getNotNull("scripts/actions/admin.L2DoorInstance.onActionShift.htm",
					player);
	...
	public boolean OnActionShift_PetInstance(Player player, GameObject gameObject) {
		if (player != null && gameObject != null && player.getPlayerAccess().CanViewChar) {
			if (gameObject.isPet()) {
				...
				String string = HtmCache.getInstance()
						.getNotNull("scripts/actions/admin.L2PetInstance.onActionShift.htm", player);
				string = string.replaceFirst("%name%", HtmlUtils.htmlNpcName((int) petInstance.getNpcId()));
				string = string.replaceFirst("%title%", String.valueOf(
						StringUtils.isEmpty((CharSequence) petInstance.getTitle()) ? "Empty" : petInstance.getTitle()));
				string = string.replaceFirst("%level%", String.valueOf(petInstance.getLevel()));
				string = string.replaceFirst("%class%",
						petInstance.getClass().getSimpleName().replaceFirst("L2", "").replaceFirst("Instance", ""));
				int var10002 = petInstance.getLoc().x;
				string = string.replaceFirst("%xyz%",
						var10002 + " " + petInstance.getLoc().y + " " + petInstance.getLoc().z);
				...
				string = string.replaceFirst("%controlItemId%",
						String.valueOf(petInstance.getControlItem().getItemId()));
				...
				string = string.replaceFirst("%dist%",
						String.valueOf((int) petInstance.getRealDistance((GameObject) player)));
		...
		if (player != null && gameObject != null && player.getPlayerAccess().CanViewChar) {
			if (gameObject.isSummon()) {
				...
				if (Config.DEBUG) {
					...
					System.out.println((Object) summonInstance.getAI().getAttackTarget());
					System.out.println(summonInstance.getAI().getNextAction());
					System.out.println();
				}
				String string = HtmCache.getInstance()
						.getNotNull("scripts/actions/admin.L2SummonInstance.onActionShift.htm", player);
				...
				string = string.replaceFirst("%class%", ((Object) ((Object) this)).getClass().getSimpleName()
						.replaceFirst("L2", "").replaceFirst("Instance", ""));
				int var10002 = summonInstance.getLoc().x;
				string = string.replaceFirst("%xyz%",
						var10002 + " " + summonInstance.getLoc().y + " " + summonInstance.getLoc().z);
				...
				string = string.replaceFirst("%dist%",
						String.valueOf((int) summonInstance.getRealDistance((GameObject) player)));
	...
	public boolean OnActionShift_ItemInstance(Player player, GameObject gameObject) {
		if (player != null && gameObject != null && player.getPlayerAccess().CanViewChar) {
			if (gameObject.isItem()) {
				...
				String string = HtmCache.getInstance()
						.getNotNull("scripts/actions/admin.L2ItemInstance.onActionShift.htm", player);
				...
				string = string.replaceFirst("%owner%",
						String.valueOf(gameObject2 == null ? "none" : gameObject2.getName()));
				string = string.replaceFirst("%ownerId%", String.valueOf(itemInstance.getOwnerId()));
				Element[] var6 = Element.VALUES;
				int var7 = var6.length;
				for (Element element : var6) {
					string = string.replaceFirst("%" + element.name().toLowerCase() + "Val%",
							String.valueOf(itemInstance.getAttributeElementValue(element, true)));
				}
	...
	private long a(long l, NpcInstance npcInstance) {
		return npcInstance.isRaid() ? (long) (Config.ALT_RAID_RESPAWN_MULTIPLIER * (double) l)
				: (long) (Config.ALT_MOBS_RESPAWN_MULTIPLIER * (double) l);
	}
}
