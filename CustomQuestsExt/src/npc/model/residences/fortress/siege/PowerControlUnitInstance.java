...
/**
 * Inner power control unit (password mini-game, powers off the Main Machine).
 * Ported from the H5 fortress siege PowerControlUnitInstance; the dwarven
 * crafter shortcut uses this core's lowercase ClassId constants.
 */
		...
		switch(cond)
		{
			case COND_ALL_OK:
				...
				if(event != null)
				{
					...
					Spawner spawn = exObject.getSpawns().get(3); // spawn of the Main Machine
					NpcInstance machine = spawn.getFirstSpawned();
					if(machine instanceof MainMachineInstance)
						((MainMachineInstance) machine).powerOff(this);
					onDecay();
				}
				break;
			case COND_TIMEOUT:
