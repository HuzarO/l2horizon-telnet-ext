...
public class SecondClassChangeItem extends ScriptItemHandler {

	@Override
	public int[] getItemIds() {
		return new int[] { 91707 };
	}

	@Override
	public boolean useItem(Playable playable, ItemInstance item, boolean arg2) {
		if (playable instanceof Player player) {
			// Check if player is in first class (level 2)
			if (player.getClassId().getLevel() != 2) {
				player.sendMessage("This scroll can only be used for second class change!");
				return false;
			}
	...
	private void showClassChangeHTML(Player player) {
		...
		sb.append("<font color=\"LEVEL\">Second Class Change</font><br1>");
		sb.append("Select your new class:<br>");
		sb.append("<font color=\"FFAA00\">Cost: 1,500,000 Adena</font><br1>");
		...
		for (ClassId cid : ClassId.VALUES) {
			// Only show level 3 classes (second class change)
			if (cid.getLevel() != 3) {
				continue;
			}
		...
		if (availableClasses.isEmpty()) {
			...
			for (ClassId cid : availableClasses) {
				...
				sb.append("<td width=135 align=center>")
						...
						.append("\" action=\"bypass second_class_change_scroll ").append(cid.getId())
