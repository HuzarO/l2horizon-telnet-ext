package ai.residences.fortress.siege;

import java.util.Collections;
import java.util.List;

import l2.gameserver.ai.Fighter;
import l2.gameserver.model.Creature;
import l2.gameserver.model.entity.events.impl.FortressSiegeEvent;
import l2.gameserver.model.entity.residence.Fortress;
import l2.gameserver.model.instances.NpcInstance;
import l2.gameserver.utils.Location;
import npc.model.residences.fortress.FortressUtils;
import npc.model.residences.fortress.siege.MercenaryCaptionInstance;

/**
 * Rebel mercenary captain AI: after the enter doors fall he walks the
 * mercenary_points route to the command post. Ported from the H5
 * ai.residences.fortress.siege.MercenaryCaption (this core has no
 * setMaxPursueRange - the template's MaxPursueRange ai_param covers it).
 */
public class MercenaryCaption extends Fighter
{
	private List<Location> _points = Collections.emptyList();
	private int _tick = -1;

	public MercenaryCaption(NpcInstance actor)
	{
		super(actor);
	}

	@Override
	public void onEvtSpawn()
	{
		super.onEvtSpawn();

		Fortress f = FortressUtils.getFortress(getActor());
		FortressSiegeEvent event = f == null ? null : f.<FortressSiegeEvent> getSiegeEvent();
		if(event == null)
			return;

		_points = event.getObjects(FortressSiegeEvent.MERCENARY_POINTS);
	}

	@Override
	protected boolean thinkActive()
	{
		NpcInstance actor = getActor();
		if(actor.isActionsDisabled())
			return true;

		if(_def_think)
		{
			if(doTask())
				clearTasks();
			return true;
		}

		if(randomWalk())
			return true;

		return false;
	}

	@Override
	protected void onEvtArrived()
	{
		super.onEvtArrived();
		if(_tick != -1)
			startMove(false);
	}

	@Override
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		_tick = -1;
		super.onEvtAttacked(attacker, damage);
	}

	public void startMove(boolean init)
	{
		if(init)
			_tick = 0;

		if(_tick == -1)
			return;

		if(_tick < _points.size())
		{
			addTaskMove(_points.get(_tick++), true);
			doTask();
		}
	}

	@Override
	public MercenaryCaptionInstance getActor()
	{
		return (MercenaryCaptionInstance) super.getActor();
	}
}
