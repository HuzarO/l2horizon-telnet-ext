...
| buyer double-click | `Action` | targeted seller with type 10: in range -> the seller's `StatusUpdate` (CUR_MP, MAX_MP) followed by the `PrivateStoreListSell` layout with the entries, so the window shows the seller's MP the way the target status does (`BuffStoreListMp` can also put the MP into the header for a client that parses it); out of range -> the core walks the buyer there and the window opens on arrival |
| Buy | `RequestPrivateStoreBuy` | `buy`: seller still type 10 and within 200, both alive, every objectId a listed entry with count 1 and the listed price, sum with overflow check, the seller can cast every bought buff like a real cast: current MP covers the MP cost (`BuffStoreConsumeMp`), the skill's consumables are in the seller's inventory (`BuffStoreConsumeItems`), the seller wields a weapon the skill allows (`BuffStoreCheckWeapon`, dances need dual swords) - refused before any adena moves otherwise, with a message to both sides; adena moved (optional tax), the MP and the consumables spent (MP without an attacker, like a workshop craft, so the seller does not enter combat stance), the seller's `StatusUpdate` CUR_MP to the seller and CUR_MP/MAX_MP to the buyer, `skill.getEffects(seller, buyer)` per buff at the seller's learned level (or the table's fixed `level`), cast animation, the stock sale messages to both sides (`S2 is sold to C1 for S3 adena` / `S2 has been purchased from C1 at S3 adena`, the item name being the buff name), `ExPrivateStoreSellingResult(dummy id, 1, buyer)` to the seller for the client's sale log, `log/buffstore` line; the store stays open |
...
| `BuffStoreConsumeItems` | True | the seller needs and spends the skill's consumables per sold buff (Spirit Ore for the Prophecies, Greater Might/Shield, Clarity, War/Earth Chant, Chant of Victory, Magnus' Chant, Victory of Pa'agrio; the Blessed Spiritshot pack for Salvation) |
